...
if ( ! class_exists( 'WC_Email_Customer_Refunded_Order_Aralco', false ) ) :
	 ...
	 *
	 * @class    WC_Email_Customer_Refunded_Order_Aralco
	 * @version  3.5.0
	 * @package  WooCommerce/Classes/Emails
	 * @extends  WC_Email
	 */
	class WC_Email_Customer_Refunded_Order_Aralco extends WC_Email {
		...
		public function __construct() {
			...
			$this->title          = __( 'Refunded order/quote', 'woocommerce' );
			$this->description    = __( 'Order/Quote refunded emails are sent to customers when their orders/quotes are refunded.', 'woocommerce' );
		...
		public function get_default_subject( $partial = false ) {
			if ( $partial ) {
				return __( 'Your {site_title} {trans_type} #{order_number} has been partially refunded', ARALCO_SLUG );
			} else {
				return __( 'Your {site_title} {trans_type} #{order_number} has been refunded', ARALCO_SLUG );
			}
		}
		...
		public function get_default_heading( $partial = false ) {
			if ( $partial ) {
				return __( 'Partial Refund: {trans_type} {order_number}', ARALCO_SLUG );
			} else {
				return __( '{trans_type} Refunded: {order_number}', ARALCO_SLUG );
			}
		}
...
return new WC_Email_Customer_Refunded_Order_Aralco();
