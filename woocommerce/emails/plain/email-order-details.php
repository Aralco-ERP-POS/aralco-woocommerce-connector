...
$text = esc_html__( '[Order #%1$s] (%2$s)', 'woocommerce' );
if($use_quote) {
    $text = esc_html__( '[Quote #%1$s] (%2$s)', ARALCO_SLUG );
}

echo wp_kses_post( wc_strtoupper( sprintf( $text, $order->get_order_number(), wc_format_datetime( $order->get_date_created() ) ) ) ) . "\n";
